...
static const uint32_t DELAY_DEBOUNCE_TIME_IN_MS = 40;
...
/* USER CODE BEGIN 0 */
/**
 * @brief carga el estado inicial de la MEF e inicializa el delay no bloqueante que usa
 * el anti-rebote
 * 
 */
void debounceFSM_init(){
	state = BUTTON_UP;
	delayInit(&delayNoBloqueante, DELAY_DEBOUNCE_TIME_IN_MS);
}
...
void debounceFSM_update(){
	...
	case BUTTON_UP:
		...
		if (readButton())
		{
			delayWrite(&delayNoBloqueante, DELAY_DEBOUNCE_TIME_IN_MS);
